...
class MainController extends Controller
{
    ...
    private function prepareQuiz($total_questions)
    {
        ...
        foreach($indexes as $index){
            ...
            $question['champion'] = $this->app_data[$index]['champion'];
            $question['correct_answer'] = $this->app_data[$index]['region'];

            // wrong answers
            $other_regions = array_column($this->app_data, 'region');
            
            // remove correct answer
            $other_regions = array_diff($other_regions, [$question['correct_answer']]);

            // shuffle the wrong answers
            shuffle($other_regions);
            $question['wrong_answers'] = array_slice($other_regions, 0, 3);
    ...
    public function game(): View
    {
        ...
        return view('game')->with([
            'champion' => $quiz[$current_question]['champion'],
